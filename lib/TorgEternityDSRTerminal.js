...
export class DSRTerminalApp extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _prepareContext(options) {    
    ...
    const context = {
      ...
      dnOptions: {
        "6": game.i18n.localize("dsrTermminal.FormTab.Step.DN.SelectOptions.6"),
        "8": game.i18n.localize("dsrTermminal.FormTab.Step.DN.SelectOptions.8"),
        "10": game.i18n.localize("dsrTermminal.FormTab.Step.DN.SelectOptions.10"),
        "12": game.i18n.localize("dsrTermminal.FormTab.Step.DN.SelectOptions.12"),
        "14": game.i18n.localize("dsrTermminal.FormTab.Step.DN.SelectOptions.14"),
        "16": game.i18n.localize("dsrTermminal.FormTab.Step.DN.SelectOptions.16"),
        "18": game.i18n.localize("dsrTermminal.FormTab.Step.DN.SelectOptions.18"),
        "20": game.i18n.localize("dsrTermminal.FormTab.Step.DN.SelectOptions.20")
      },
      formData: this.formData,
      tabs: {
        form : {id: 'form', label : game.i18n.localize("dsrTermminal.Tabs.Form.Title"), group: 'primary', icon: 'fas fa-edit'},
        archives : {id: 'archives', label : game.i18n.localize("dsrTermminal.Tabs.Form.Archives"), group: 'primary', icon: 'fas fa-archive'}
      },
      tabGroups: {
  ...
  static async _onRestore(event, target) {
    ...
    this.render();
    ui.notifications.info(`${game.i18n.localize("dsrTermminal.UiNotification.Restore")}: ${page.title}`);
  }
  ...
  static async _onSave(_event, target){
    ...
    await journal.updateEmbeddedDocuments("JournalEntryPage", [
      {
        _id: pageId,
        name: `${data.dsrTitle || game.i18n.localize("dsrTermminal.Journal.MissionTitle.Default")}`,
        text: {
          content: htmlToSave
        }
      }
    ...
    this.render()
    ui.notifications.info(game.i18n.localize("dsrTermminal.UiNotification.Save"));
  }
...
/* -- Internal Logic -- */
  async submitMission(data, isArchiveOnly) {
    ...
    if(!data.fromRestoration){   
      await journal.createEmbeddedDocuments("JournalEntryPage", [{ name: `${data.dsrTitle || game.i18n.localize("dsrTermminal.Journal.MissionTitle.Default")}`, type: "text", text: { content: html } }]);   
    }
    this.archiveEntries = null
    this.formData = {}
    this.render()
    ui.notifications.info(game.i18n.localize("dsrTermminal.UiNotification.Archive"));
  }
  ...
  async _generateChatHtml(formData) {
    ...
    const templateData = {
      title: formData.dsrTitle || game.i18n.localize("dsrTermminal.Journal.MissionID.NotFound"),
      missionId: missionId,
  ...
  async _getArchives() {
    ...
    return journal.pages.contents.slice().reverse().map(page => {
      ...
      const steps = ['a', 'b', 'c', 'd'].map(letter => {
        ...
        if (!m) return { label: letter.toUpperCase(), summary: game.i18n.localize("dsrTermminal.Bypassed"), details: "", bypassed: true };
        return {
