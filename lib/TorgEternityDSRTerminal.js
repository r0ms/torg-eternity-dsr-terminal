...
export class DSRTerminalApp extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _prepareContext(options) {    
    ...
    const tabGroupPrimary = this.tabGroups?.primary ?? 'form'
    const context = {
      steps: ['A', 'B', 'C', 'D'],
      skills: CONFIG.torgeternity.skills,
      attributes: CONFIG.torgeternity.attributeTypes,
      // TODO: Check if it's worth using CONFIG.toreternity.dnTypes instead of hardcoded and readapt archives accordingly
      dnOptions: {
        "6": game.i18n.localize("dsrTerminal.FormTab.Step.DN.SelectOptions.6"),
        "8": game.i18n.localize("dsrTerminal.FormTab.Step.DN.SelectOptions.8"),
        "10": game.i18n.localize("dsrTerminal.FormTab.Step.DN.SelectOptions.10"),
        "12": game.i18n.localize("dsrTerminal.FormTab.Step.DN.SelectOptions.12"),
        "14": game.i18n.localize("dsrTerminal.FormTab.Step.DN.SelectOptions.14"),
        "16": game.i18n.localize("dsrTerminal.FormTab.Step.DN.SelectOptions.16"),
        "18": game.i18n.localize("dsrTerminal.FormTab.Step.DN.SelectOptions.18"),
        "20": game.i18n.localize("dsrTerminal.FormTab.Step.DN.SelectOptions.20")
      },
      formData: this.formData,
      tabs: {
        form : {id: 'form', label : game.i18n.localize("dsrTerminal.Tabs.Form.Title"), group: 'primary', icon: 'fas fa-edit'},
        archives : {id: 'archives', label : game.i18n.localize("dsrTerminal.Tabs.Form.Archives"), group: 'primary', icon: 'fas fa-archive'}
      },
      tabGroups: {
  ...
  static async _onRestore(event, target) {
    ...
    this.render();
    ui.notifications.info(`${game.i18n.localize("dsrTerminal.UiNotification.Restore")}: ${page.title}`);
  }
  ...
  static async _onSave(_event, target){
    const data = new foundry.applications.ux.FormDataExtended(this.element.querySelector("form")).object;
    const journal = game.journal.getName(JOURNAL_NAME) || await JournalEntry.create({ name: JOURNAL_NAME });
    const htmlToSave = await this._generateChatHtml(data);
    const pageId = data.restorationId
    await journal.updateEmbeddedDocuments("JournalEntryPage", [
      {
        _id: pageId,
        name: `${data.dsrTitle || game.i18n.localize("dsrTerminal.Journal.MissionTitle.Default")}`,
        text: {
          content: htmlToSave
        }
      }
    ...
    this.archiveEntries = null
    this.formData = {}
    this.tabGroups.primary = "archives";
    this.render()
    ui.notifications.info(game.i18n.localize("dsrTerminal.UiNotification.Save"));
  }
...
/* -- Internal Logic -- */
  async submitMission(data, isArchiveOnly) {
    ...
    if(!data.fromRestoration){   
      await journal.createEmbeddedDocuments("JournalEntryPage", [{ name: `${data.dsrTitle || game.i18n.localize("dsrTerminal.Journal.MissionTitle.Default")}`, type: "text", text: { content: html } }]);   
    }
    this.archiveEntries = null
    this.formData = {}
    this.render()
    ui.notifications.info(game.i18n.localize("dsrTerminal.UiNotification.Archive"));
  }
  ...
  async _generateChatHtml(formData) {
    ...
    const templateData = {
      title: formData.dsrTitle || game.i18n.localize("dsrTerminal.Journal.MissionTitle.Default"),
      missionId: missionId,
 ...
 _generateCheckEnricher = (skill, attribute, dn, description, unskillUse = false) => {
  ...
  _parseContentForRestoration(html) {
    ...
    let data = {
        dsrTitle: doc.querySelector('h2')?.textContent.trim() || game.i18n.localize("dsrTerminal.Journal.MissionTitle.Default"),
  ...
  async _getArchives() {
    ...
    return journal.pages.contents.slice().reverse().map(page => {
      ...
      const steps = ['a', 'b', 'c', 'd'].map(letter => {
        ...
        if (!m) return { label: letter.toUpperCase(), summary: game.i18n.localize("dsrTerminal.Bypassed"), details: "", bypassed: true };
        return {
